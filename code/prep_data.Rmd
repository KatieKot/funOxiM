...
# Path to uniq genes file
path2uniq <- "./input/gene.stat"
...
# This one is based on DNA matches (BLAST again)
uniqunes <- fread(path2uniq) %>% 
  setnames(., c("proteinID", "gene_name", "uniqness") )
uniqunes$proteinID <- as.character(uniqunes$proteinID)
uniqunes <- merge(uniqunes, paralog, all.y=TRUE) %>% 
...
d <- ID_dalis %>% 
  ...
  merge(., uniqunes[, c("proteinID", "uniqness", "paral"), with=FALSE], by="proteinID", all=FALSE) %>% 
